...
- **Co-located tests:** `*.test.ts` (or `*.test.tsx`) files next to the source file they target — the default for anything that tests a specific file. Cover branded constructors, Zod schemas, parsers, formatters, hook behavior, component interactions, and app-shell wiring (e.g. `App.test.tsx` beside `App.tsx`, even though it mounts the full provider tree). The trigger is "targets one file," not "is a narrow unit test."
...
- 2026-05-31 — Reframed the § Testing "Unit tests / Integration tests" split as "Co-located tests / Flow tests" so the placement rule keys off *what a test targets* rather than its breadth: a test of a specific source file co-locates beside it (even an app-shell test that mounts the full tree), and only cross-screen flows that map to no single file live in `src/__tests__/`. Removes the contradiction with `docs/coding-standards/typescript.md` § 12 that Issue [#206](https://github.com/brendanbyrne/beerio-kart/issues/206) flagged. Companion to that Issue, which moved `App.test.tsx` and `routing.test.tsx` (both target the `App` module) out of `src/__tests__/` and added the matching § 12 carve-out.
