...
- **Co-located tests:** `*.test.ts` (or `*.test.tsx`) files next to the source file they target — the default for anything that tests a specific file. Cover branded constructors, Zod schemas, parsers, formatters, hook behavior, component interactions, and app-shell wiring (e.g. `App.test.tsx` beside `App.tsx`, even though it mounts the full provider tree). The trigger is "targets one file," not "is a narrow unit test." Name the test after that source (`Foo.test.tsx`), with an optional concern infix (`App.routing.test.tsx`) when one source has several focused test files.
- **Flow / integration tests:** End-to-end tests that span multiple screens and don't map to a single source file live under `frontend/src/__tests__/`, using React Testing Library + MSW to mock the API. Cover the major user flows (login → create session → submit run, etc.). The directory may not exist until the first such flow test is written.
...
- 2026-05-31 — Reframed the § Testing "Unit tests / Integration tests" split as "Co-located tests / Flow tests" so the placement rule keys off *what a test targets* rather than its breadth: a test of a specific source file co-locates beside it (even an app-shell test that mounts the full tree), and only cross-screen flows that map to no single file live in `src/__tests__/`. Added the filename-mirrors-source convention (with an optional concern infix). Removes the contradiction with `docs/coding-standards/typescript.md` § 12 that Issue [#206](https://github.com/brendanbyrne/beerio-kart/issues/206) flagged. Companion to that Issue, which moved the two App-module tests out of `src/__tests__/` and (on PR review) renamed `routing.test.tsx` → `App.routing.test.tsx` for the explicit pairing.
