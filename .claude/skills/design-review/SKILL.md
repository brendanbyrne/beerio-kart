...
description: >
  ...
  adherence to docs/design.md conventions. Use this skill whenever the user wants an architecture review,
  ...
  for design", "does this match docs/design.md", "architecture review", "check these changes against the
...
Claude Code handles code quality, security, and database query patterns via its own code-review skill. Your focus is the bigger picture: does this change align with docs/design.md? Does it introduce architectural drift? Are the API contracts right? Is the data model being used correctly?
...
- `docs/design.md` (to verify conventions)
...
The data model in docs/design.md is the contract. Check changes against it:
...
- **Nullability.** The project defaults to NOT NULL. Every nullable column should have a clear justification documented in docs/design.md. If a new nullable column appears without one, flag it.
...
Compare new or changed endpoints against the API section of docs/design.md:
...
docs/design.md defines three core principles. Evaluate changes against them:
...
- **Earmarked decisions aren't prematurely resolved.** docs/design.md marks some decisions as "earmarked for discussion" (e.g., shared leaderboard component, showing both previous and preferred setup). If a PR resolves one of these without discussion, call it out.

### Migration Safety (design-level)

The code-review skill checks migration syntax. You check migration semantics:

- **Does the migration match docs/design.md?** If the schema in the migration doesn't match what docs/design.md describes, one of them is wrong. Figure out which.
...
- **Design findings**, grouped:
  - **Misalignment** (contradicts docs/design.md): Data model violations, API contract breaks, principle violations
...
### docs/design.md Updates (when needed)

If the review reveals that docs/design.md is outdated or that a design decision should be recorded, draft the specific additions or changes. Don't just say "update docs/design.md" — write the text.
...
- Distinguish "wrong" from "different." If a change contradicts docs/design.md, that's concrete. If it makes a reasonable choice that docs/design.md doesn't cover, frame it as a discussion point, not a defect.
