...
> **Status: complete.** Archived 2026-05-15. All numbered PRs (Streams A–J) plus PR-H1+ are signed off. Retained for historical reference; live conventions live in [`../../coding-standards/`](../../coding-standards/).
...
- **Sign-off:** [x] Audit completed 2026-04-15 ([`docs/designs/2026-04-15-rust-audit.md`](./2026-04-15-rust-audit.md)).
...
- **Sign-off:** [x] Closed 2026-05-14 via PR [#155](https://github.com/brendanbyrne/beerio-kart/pull/155).
...
- **Scope:**
  - Add `rstest` and `insta` as `[dev-dependencies]`.
...
- **Deferred to PR-D1/D2:** `proptest` was originally in this PR's scope but moved to whichever D-stream PR introduces the first round-trip invariant test (typically PR-D1 / [#122](https://github.com/brendanbyrne/beerio-kart/issues/122) on a UUID round-trip, or PR-D2 / [#133](https://github.com/brendanbyrne/beerio-kart/issues/133) on a string-newtype invariant). Per `rust.md` § 7, `proptest` is reserved for algebraic invariants — adding it here with no consumer was the "introduce dep before there's a need" pattern the project pushes back on, and adding it in the PR that needs it makes the rationale obvious in `git blame`. The PR-D1 and PR-D2 Issue bodies carry a scope bullet pointing here.
...
- **Sign-off:** [x] Closed 2026-05-14 via PR [#150](https://github.com/brendanbyrne/beerio-kart/pull/150).
...
- **Sign-off:** [x] Closed 2026-05-15. All PR-A1 scaffolding allows cleared via sub-PRs [#108](https://github.com/brendanbyrne/beerio-kart/pull/108) (`unwrap_used`/`expect_used`), [#111](https://github.com/brendanbyrne/beerio-kart/pull/111) (10 mechanical pedantic lints), [#112](https://github.com/brendanbyrne/beerio-kart/pull/112) (`module_name_repetitions` + casts); Issues [#102](https://github.com/brendanbyrne/beerio-kart/issues/102) and [#98](https://github.com/brendanbyrne/beerio-kart/issues/98) verified clean without code changes (PR-A1 never introduced those module-level allows). Parent tracker [#95](https://github.com/brendanbyrne/beerio-kart/issues/95) closed. The `#[allow(clippy::...)]` annotations that remain in the codebase are principled per-item or test-policy exceptions (e.g., `unused_async` on axum middleware fns; `unwrap_used`/`expect_used`/`panic` in tests per `rust.md` § 11), not scaffolding. New `#[allow]`s that appear later are handled in their introducing PR's review, not via this stream.
...
- **Scope:**
  ...
  - Findings get posted as GitHub PR review comments (line-anchored where possible) per [`docs/designs/2026-05-04-design-doc-restructure.md`](./2026-05-04-design-doc-restructure.md) §8.8.
...
- **Sign-off:** [x] Closed 2026-05-13 via Issue [#131](https://github.com/brendanbyrne/beerio-kart/issues/131). Scope landed across two direct-to-main doc commits: `f6b51b4` (2026-05-02 — skill rewrite for per-area layout: README.md-first, diff→area-file table) and `6d3c001` (2026-05-05 — single-review POST with line-anchored inline comments as default). The §8.8 reference cleanup landed via PR [#90](https://github.com/brendanbyrne/beerio-kart/pull/90) (2026-05-08).

---

## Stream J — Codegen workflow

The architectural decision to switch from codegen-driven entities to hand-written entities is recorded in [`docs/designs/2026-05-02-entity-codegen-strategy.md`](./2026-05-02-entity-codegen-strategy.md). These two PRs implement that decision.
...
- 2026-05-04 — Added Stream J (Codegen workflow) covering PR-X1 / PR-X2; updated PR-D3 and PR-E3 to drop the `just entities` regen step (entities are now committed source). Closes the codegen-strategy decision recorded at [`docs/designs/2026-05-02-entity-codegen-strategy.md`](./2026-05-02-entity-codegen-strategy.md). PR-X1.
...
- 2026-05-05 — Renamed `Phase A`–`Phase J` to `Stream A`–`Stream J` throughout to free the `Phase` namespace for build phases only, per the cup-name milestone convention adopted in the design record's 2026-05-05 amendment ([`docs/designs/2026-05-04-design-doc-restructure.md`](./2026-05-04-design-doc-restructure.md) §12.5 #2).
...
- 2026-05-08 — Updated PR-I1 scope: "Findings get written to `reviews/pr/`" → "Findings get posted as GitHub PR review comments per [`docs/designs/2026-05-04-design-doc-restructure.md`](./2026-05-04-design-doc-restructure.md) §8.8." `reviews/pr/` was retired in the docs restructure; live-prose references should match the new convention. Closes part of [#89](https://github.com/brendanbyrne/beerio-kart/issues/89).
...
- 2026-05-10 — Marked PR-A2 sign-off complete (audit completed 2026-04-15; deliverable is [`docs/designs/2026-04-15-rust-audit.md`](./2026-04-15-rust-audit.md)).
...
- 2026-05-14 — Marked PR-F4 sign-off complete (PR [#155](https://github.com/brendanbyrne/beerio-kart/pull/155) — Issue [#123](https://github.com/brendanbyrne/beerio-kart/issues/123)). Resolved the design question on the variant: added a dedicated `Error::Timeout { budget }` variant mapped to `504 Gateway Timeout` (not `Error::Internal` → 500), so operators can chart timeouts independently of generic 500-class failures. The 504 status is semantically apt despite SQLite being in-process — it is the "upstream" the per-call timeout protects against. Resolved the helper question: function wrappers `timeout::db_query` (2 s, single statements) and `timeout::db_txn` (5 s, transaction-frame statements) rather than a `db_timeout!` macro, since tracing spans from PR-F5 already supply call-site context and functions compose better with `?` and `rustfmt`. Per-statement budgeting applies inside transactions too — `db_txn` only bounds the `begin` / `commit` / `rollback` statements themselves; the body's individual statements each get their own `db_query` budget. Verification test (`backend/tests/db_timeout.rs`) holds a real SQLite file write lock and asserts a wrapped contending writer fires at ~2 s rather than the pool's 5 s `busy_timeout`. `tokio.md` § 12's stale `AppError::Internal("db timeout".into())` example was replaced with the new helpers in the same PR. The remaining open compliance-plan row is PR-I1 (code-review skill update).
- 2026-05-15 — Filed Issue [#157](https://github.com/brendanbyrne/beerio-kart/issues/157) (`code` field rollout) and closed Issue [#146](https://github.com/brendanbyrne/beerio-kart/issues/146) (typed-Path-extractor 400s) as subsumed by it. Scope analysis on #146 showed the Path/Json extractor fix couldn't cleanly land alone — wiring just the new extractor's `code` field would have left one path emitting `code` while every other 4xx/5xx response did not (partial rollout without enabling the actual switchboard the field exists for). Bundling into #157 means the wire-format `code` field becomes real across the entire 4xx/5xx surface in one PR, and the custom Path/Json extractors plug in as the matching 400-emitting sites for `invalid_path_param` / `invalid_request_body`. Design choices (hybrid helper API, `ErrorCode` enum with one variant per registry row, 422 deferred) captured in ADR [0036](../../decisions/0036-error-code-rollout.md). #157 is not part of the original compliance-plan PR sequence (PR-F4 was the last numbered slice) but lands within the same `Hardening: Backend compliance plan` milestone — completing the long-deferred § 3 contract is compliance work even if not explicitly enumerated in the original plan.
- 2026-05-15 — Marked PR-H1+ sign-off complete. The row had been intentionally open-ended ("Created lazily; one row per PR as we go") to accommodate future allows, but its stated scope — clearing PR-A1's scaffolding allows — is fully discharged: three sub-PRs landed (#108, #111, #112) and two verifications confirmed PR-A1 never added the allows the Issue bodies named (#102 nursery, #98 nursery again). Parent tracker [#95](https://github.com/brendanbyrne/beerio-kart/issues/95) closed 2026-05-11. Grep across `backend/src/` and `backend/tests/` shows the remaining `#[allow(clippy::...)]` sites are principled: 2 narrow per-item allows in source code with the lint disagreeing with intentional code (axum's `unused_async` rule on middleware fns; `struct_field_names` on `RunSubmission`), the standard test-only `unwrap_used`/`expect_used`/`panic` triplet per `rust.md` § 11 (wired via `cfg_attr(test, ...)` in `lib.rs` plus one `#![allow(...)]` per integration-test binary since `cfg_attr(test, ...)` doesn't reach `tests/*.rs`), and four workspace-level `[workspace.lints]` carve-outs in `Cargo.toml` for lints that conflict with project realities. Going forward, any new `#[allow]` is handled in the introducing PR's review and any cleanup is tracked at that PR's level, not as a continuation of this stream. With this, every row in the compliance plan is signed off — Stream A through Stream J, all complete.
- 2026-05-15 — Marked PR-I1 sign-off complete. Issue [#131](https://github.com/brendanbyrne/beerio-kart/issues/131) closed 2026-05-13 as COMPLETED with no linking PR; the row's checkbox had been stale since. All three scope bullets verified in [`.claude/skills/code-review/SKILL.md`](../../../.claude/skills/code-review/SKILL.md): (1) `coding-standards/README.md` is item #1 in Required reading (SKILL.md:32); (2) the "Diff touches → Read" table maps diff areas to individual standards files (SKILL.md:38-44); (3) Tier 2 posts a single PR review with line-anchored inline comments and cites `designs/2026-05-04-design-doc-restructure.md` §8.8 directly (SKILL.md:119-161, §8.8 named at line 161). Implementation history: `f6b51b4` (2026-05-02, direct-to-main — skill rewrite for per-area layout) and `6d3c001` (2026-05-05, direct-to-main — single-review POST default); §8.8 reference cleanup via PR [#90](https://github.com/brendanbyrne/beerio-kart/pull/90) (2026-05-08). With this, every numbered compliance-plan PR (PR-A1 through PR-X2) is signed off. PR-H1+ remains as the lazy lint-cleanup stream — created per-PR as new `#[allow]`s arise.
