...
> **Status: complete.** Archived 2026-06-15. All 16 PRs merged and signed off; retained for historical reference. Live frontend conventions live in [`../../coding-standards/`](../../coding-standards/).
...
**Issue:** [#184](https://github.com/brendanbyrne/beerio-kart/issues/184) · **Merged PR:** [#221](https://github.com/brendanbyrne/beerio-kart/pull/221)
...
**Issue:** [#185](https://github.com/brendanbyrne/beerio-kart/issues/185) · **Merged PR:** [#235](https://github.com/brendanbyrne/beerio-kart/pull/235)
...
- **Scope:**
  ...
  - **Flip remaining lint rules from `warn` to `error`.** PR-A1 started several rules at `warn` so existing code wouldn't block CI; later PRs remove the offenders. By PR-H1, audit `eslint.config.js`'s warn-level blocks and flip every rule whose offenders are gone — `consistent-type-definitions`, `import/consistent-type-specifier-style`, and the `strictTypeChecked` family (`no-unsafe-*`, `no-floating-promises`, `no-misused-promises`, `no-unnecessary-condition`, `no-confusing-void-expression`, etc.). `import/no-default-export` and `no-explicit-any` / `no-non-null-assertion` are flipped earlier (PR-D1, PR-F1). Any rule that still can't be flipped means an earlier PR missed its scope.
    - **Update (post-#225, 2026-05-31):** Issue [#217](https://github.com/brendanbyrne/beerio-kart/issues/217) / PR [#225](https://github.com/brendanbyrne/beerio-kart/pull/225) — the test-assertion tightening pass, *not* a plan PR — already flipped the clean `strictTypeChecked` rules to `error` (`no-unsafe-*`, `no-deprecated`, `prefer-regexp-exec`, `no-unnecessary-type-assertion`) and added the `@vitest/eslint-plugin` + `eslint-plugin-testing-library` presets. So PR-H1's remaining flip-scope is the rules that still have offenders: `consistent-type-definitions`, `import/consistent-type-specifier-style`, `no-floating-promises`, `no-misused-promises`, `no-unnecessary-condition`, `no-confusing-void-expression`, `no-unnecessary-type-arguments`, `prefer-nullish-coalescing`, `restrict-template-expressions`; the two test-preset rules (`vitest/no-conditional-expect`, `testing-library/no-node-access`) were since flipped to `error` via [#227](https://github.com/brendanbyrne/beerio-kart/issues/227) (landed 2026-06-15), so they no longer count against PR-H1's remaining scope.
...
| 15 | G2 | Accessibility sweep | G1 | [#184](https://github.com/brendanbyrne/beerio-kart/issues/184) | [x] |
| 16 | H1 | Lint cleanup + drift-check CI + test-coverage backfill | All | [#185](https://github.com/brendanbyrne/beerio-kart/issues/185) | [x] |
...
- 2026-05-31 — PR-G2 ([#184](https://github.com/brendanbyrne/beerio-kart/issues/184)) merged as [#221](https://github.com/brendanbyrne/beerio-kart/pull/221). Flipped its per-section sign-off `[ ]` → `[x]` and the row-15 table checkbox, and added the **Merged PR** link. Scope deltas worth recording: (1) the two modals' a11y is implemented via a new reusable `useModalA11y` hook (focus trap + Escape + focus-restore) rather than native `<dialog>` — the plan's Risk note suggested `<dialog>` as the escape hatch, but jsdom doesn't implement `showModal()`'s trap/Escape/restore, so native would make the keyboard behaviors untestable (the testing standard forbids assertions that can't fail); `react.md` § 10 gained a pointer to the hook in-PR on reviewer suggestion. (2) Focus management for `RunEntrySheet`'s nested drink/setup sub-pickers was **descoped** — the plan's scope named only the two top-level modals — and split out to [#222](https://github.com/brendanbyrne/beerio-kart/issues/222) (Ready/Medium): the sub-pickers render as opaque overlays atop the still-mounted sheet, so they need `inert`-the-sheet + focus-into/restore handling a future PR will add. (3) Beyond the literal scope bullets, accessible names were added to the time/drink-name/password inputs and the DQ slider was made keyboard-operable (`role="button"` + Enter/Space) — both required to reach "axe zero violations on the main flows" and to flip `jsx-a11y/no-static-element-interactions` to error. (4) All four held-back `jsx-a11y` rules (`no-static-element-interactions`, `click-events-have-key-events`, `label-has-associated-control`, `no-autofocus`) are now `error`. G2 closes Stream G; the only remaining open item is H1 ([#185](https://github.com/brendanbyrne/beerio-kart/issues/185)). Once H1 merges, this plan is archive-eligible (`docs/CLAUDE.md` § Design records → Archive).
- 2026-05-31 — Out-of-band lint progress noted in PR-H1's scope: Issue [#217](https://github.com/brendanbyrne/beerio-kart/issues/217) / PR [#225](https://github.com/brendanbyrne/beerio-kart/pull/225) (the test-assertion tightening pass, *not* a plan PR) already flipped the clean `strictTypeChecked` rules (`no-unsafe-*`, `no-deprecated`, `prefer-regexp-exec`, `no-unnecessary-type-assertion`) `warn` → `error` and added the `@vitest/eslint-plugin` + `eslint-plugin-testing-library` presets. PR-H1's remaining flip-scope shrinks to the still-offending rules; the two test-preset rules left at `warn` are tracked in [#227](https://github.com/brendanbyrne/beerio-kart/issues/227). No sign-off change — H1 is still open.
- 2026-06-15 — PR-H1 ([#185](https://github.com/brendanbyrne/beerio-kart/issues/185)) merged as [#235](https://github.com/brendanbyrne/beerio-kart/pull/235). Flipped its per-section sign-off `[ ]` → `[x]` and the row-16 table checkbox, and added the **Merged PR** link. H1 was the last item — **the plan is now fully signed off with all 16 PRs merged, so it (and its companion `2026-05-16-frontend-audit.md`) moved to `archive/`** (`docs/CLAUDE.md` § Design records → Archive). PR-H1 deltas worth recording: the DTO drift check watches an explicit path list (no single `backend/src/dtos/` dir — DTOs span `routes/`, `domain/enums.rs`, and several `services/` modules; the watch-list initially missed `SessionSummary` / `AccessClaims`, caught on review); the Codecov "blocking" gate flip reports a ✗ on regression but stays advisory until `main` gets a branch-protection rule (the always-runs-vs-required-check wiring is tracked in [#195](https://github.com/brendanbyrne/beerio-kart/issues/195)).
