...
- **Scope:**
  ...
  - **Flip remaining lint rules from `warn` to `error`.** PR-A1 started several rules at `warn` so existing code wouldn't block CI; later PRs remove the offenders. By PR-H1, audit `eslint.config.js`'s warn-level blocks and flip every rule whose offenders are gone — `consistent-type-definitions`, `import/consistent-type-specifier-style`, and the `strictTypeChecked` family (`no-unsafe-*`, `no-floating-promises`, `no-misused-promises`, `no-unnecessary-condition`, `no-confusing-void-expression`, etc.). `import/no-default-export` and `no-explicit-any` / `no-non-null-assertion` are flipped earlier (PR-D1, PR-F1). Any rule that still can't be flipped means an earlier PR missed its scope.
    - **Update (post-#225, 2026-05-31):** Issue [#217](https://github.com/brendanbyrne/beerio-kart/issues/217) / PR [#225](https://github.com/brendanbyrne/beerio-kart/pull/225) — the test-assertion tightening pass, *not* a plan PR — already flipped the clean `strictTypeChecked` rules to `error` (`no-unsafe-*`, `no-deprecated`, `prefer-regexp-exec`, `no-unnecessary-type-assertion`) and added the `@vitest/eslint-plugin` + `eslint-plugin-testing-library` presets. So PR-H1's remaining flip-scope is the rules that still have offenders: `consistent-type-definitions`, `import/consistent-type-specifier-style`, `no-floating-promises`, `no-misused-promises`, `no-unnecessary-condition`, `no-confusing-void-expression`, `no-unnecessary-type-arguments`, `prefer-nullish-coalescing`, `restrict-template-expressions`; the two test-preset rules still at `warn` (`vitest/no-conditional-expect`, `testing-library/no-node-access`) are tracked in [#227](https://github.com/brendanbyrne/beerio-kart/issues/227).
