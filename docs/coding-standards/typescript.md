...
- **Rule:** Use [Vitest](https://vitest.dev) for unit and component tests. A test that targets a specific source file lives in `*.test.ts` (or `*.test.tsx` for components) next to that file — not in a sibling `__tests__/` directory. This holds even when the test renders a broad tree to exercise one file's behavior (e.g. `App.test.tsx` beside `App.tsx`). The sole exception is end-to-end flow tests that span multiple screens and don't map to a single source file: those live in `src/__tests__/` (see [`react.md`](./react.md) § 13).
...
- 2026-05-31 — Clarified the § 12 co-location rule so it no longer reads as an absolute ban on `__tests__/` (which contradicted `react.md` § 13 and `frontend/CLAUDE.md` § Testing, both of which reserve `src/__tests__/` for end-to-end flow tests). The co-location trigger is now "a test that targets a specific source file" — which holds even when the test mounts a broad tree (e.g. `App.test.tsx` beside `App.tsx`) — with the sole exception being cross-screen flow tests that don't map to one file. Companion to Issue [#206](https://github.com/brendanbyrne/beerio-kart/issues/206), which moved `App.test.tsx` and `routing.test.tsx` out of `src/__tests__/` (both target the `App` module, not a user flow) and updated `frontend/CLAUDE.md` § Testing to match.
