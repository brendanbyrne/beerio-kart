...
- **Rule:** A `.rs` file's *non-test* code stays under ~500 lines. Past that, split by concern. Tests in `#[cfg(test)] mod tests` do not count toward the limit.
  ...
  - **Example (split):** `services/sessions.rs` → `services/sessions/lifecycle.rs`, `services/sessions/rulesets.rs`, `services/sessions/cleanup.rs`, with `services/sessions.rs` as the module root.
