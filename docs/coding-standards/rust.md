...
- **Rule:** Wrap foreign error types from infrastructure crates as typed `#[from]` variants rather than folding them into `Internal(anyhow::Error)`. The repo's `Token(#[from] jsonwebtoken::errors::Error)` and `Hash(#[from] argon2::password_hash::Error)` are the examples — both still map to 500 / `internal`, but staying typed lets `?` work directly on token and password-hash operations, and the wrapped error stays reachable via `error.source()` so the boundary log captures the real cause.
  - **Why:** Collapsing them at every call site is noisier and discards the `#[from]` ergonomics; a typed variant is one `?` away and still logs the underlying failure.

- **Rule:** Use the project-local `extract::Path<T>` / `extract::Json<T>` (not `axum::extract::Path` / `axum::Json`) at every route. Their rejections emit the standard `{ "error", "code" }` envelope — `invalid_path_param` for a bad path segment, `invalid_request_body` for a JSON parse/validation failure (see [`../api-contract.md`](../api-contract.md) § 7) — instead of Axum's default plain-text rejection bodies.
  - **Why:** A route on Axum's stock extractors leaks a non-conforming plain-text 400 on malformed input. Routing every extraction through the project-local wrappers keeps the wire contract uniform.
...
- **Rule:** The observability stack is `tracing` (the logging facade used throughout app code) + `tracing-subscriber` (with the `fmt` and `env-filter` features — formats and emits the output) + `tower-http`'s `TraceLayer` (automatic per-request HTTP logging: method, path, status, duration). Verbosity is controlled by the `RUST_LOG` env var, defaulting to `info` when unset; `RUST_LOG=beerio_kart=debug` turns on debug for application code while leaving dependencies at info.
  - **Why:** One place documents which knobs to turn in dev and prod. The `env-filter` feature is what makes per-crate `RUST_LOG` directives work; `TraceLayer` is what gives you request logs without hand-instrumenting every handler.
...
- 2026-05-31 — §§ 1, 10: absorbed three observability/error facts orphaned by the design.md slim-down (#220/#223). § 1 gained rules for typed `#[from]` wrapper variants (`Token`/`Hash` around jsonwebtoken/argon2) and the project-local `extract::Path`/`extract::Json` envelope convention; § 10 gained the observability-stack + `RUST_LOG` rule (`tracing` / `tracing-subscriber` `env-filter` / `tower-http::TraceLayer`; default `info`). These were the only facts in design.md's deleted `## Observability` block not already canonical here, in `api-contract.md` §§ 2/7, `seaorm.md` § 7, `tokio.md` § 12, or ADR 0036.
