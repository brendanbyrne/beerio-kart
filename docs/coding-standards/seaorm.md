...
- **Rule:** Use the builder API (`Entity::find()`, `find_by_id`, `ActiveModel::insert` / `update`) for single-table reads and writes. Drop to raw SQL only for the cases § 10 lists (multi-table JOINs the builder can't express cleanly, window functions, recursive CTEs) — never hand-roll SQL for single-table ops.
  - **Why:** The builder is checked against the entity definitions, so a column rename becomes a compile error rather than a runtime surprise — type safety and refactor-proofness for free. Raw SQL trades that away for expressiveness; only pay it where the builder genuinely falls short.
...
**"Launch" definition.** "Launch" in the migration policy means *the moment we want to preserve database data between deployments*. As of May 2026, we have no real data and the schema is still in flux — we're prelaunch. Once we deploy a version where dropping the DB to apply schema changes would lose data we care about, we flip to append-only migrations and the consolidated migration becomes immutable history. This section will be updated at that time.

- **Rule:** While prelaunch (data is throwaway), edit the single consolidated migration file in place (currently `backend/migration/src/m20260101_000001_initial_schema.rs`) — adding a table, column, index, or constraint modifies that file; never append a new migration file. After a schema edit, reset the dev DB (delete the local SQLite file under `data/db/`, or run a `dev-reset` task if one exists) so SeaORM recreates the schema and the seeder repopulates on next start; recreate any meaningful local data via seed/fixtures, not by hand. Code that depends on the schema — entities, services, tests — changes in the **same PR** as the migration edit.
  - **Why:** Pre-real-data, the audit history of a migration doesn't earn its keep. One file with the current schema is simpler than N append-only files when there's no data to preserve. The local-reset workflow is spelled out in [`data/README.md`](../../data/README.md).
  - **Source:** Project policy (prelaunch).
...
- **Rule:** When changing an existing table, edit the migration and the entity (and any service / DTO code that depends on the schema) in the same PR. Reset the dev DB after the migration edit per § 5 (prelaunch schema rule).
...
- 2026-05-15 — Updated the Companions list to drop the now-archived `../compliance-plan.md` (called out separately as an archived companion); updated the two § 6 inline links to the entity-codegen-strategy record to point at its archived path (`../designs/archive/2026-05-02-entity-codegen-strategy.md`). Companion to PR [#160](https://github.com/brendanbyrne/beerio-kart/pull/160) / Issue [#159](https://github.com/brendanbyrne/beerio-kart/issues/159).
- 2026-05-31 — § 5 became the canonical home for the prelaunch schema-change policy (#220). Absorbed the operating detail that was sole-owned by `backend/CLAUDE.md` § Schema changes — the dev-DB reset mechanic, no-data-preservation, and the same-PR rule — and repointed the backward citations: the "Launch" definition's "CLAUDE.md will be updated" note and the § 5 `Source` / § 6 cross-reference no longer point at CLAUDE.md. backend/CLAUDE.md § Schema changes is now a pointer here, and the three ADRs that cited `backend/CLAUDE.md § schema-changes-prelaunch` (0035, 0037, 0038) now cite this section.
- 2026-05-31 — § 2 gained an explicit "builder API for single-table ops; raw SQL only where § 10 allows" rule, making §§ 2/10 the canonical home for ORM-usage guidance (#220/#223). The rule previously lived in `design.md` § ORM Usage (now removed) and was duplicated in `backend/CLAUDE.md` § ORM usage (now a pointer here).
