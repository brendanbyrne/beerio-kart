...
| `Timeout { budget }` | 504 | Generic `"Request timed out"` |
...
- `Timeout { budget }` is the operational signal for a stuck call. Raised only by `timeout::db_query` (2 s) / `timeout::db_txn` (5 s) when the wrapped `tokio::time::timeout` budget elapses; maps to **504 Gateway Timeout** with the generic `"Request timed out"` body. The elapsed `budget` is logged via `tracing::warn!(budget_ms = …)` for operators but is never returned to the client. Distinct from `Internal` so a stuck query (operational pain) doesn't get bucketed with invariant violations or driver crashes (bug-class) — operators can chart timeout rates separately from generic 500-class failures. SQLite is the "upstream" in the 504 sense even though the database is in-process; the proxy-flavoured status code is semantically apt because the per-call timeout is exactly what a 504 protects against. See [`coding-standards/tokio.md`](./coding-standards/tokio.md) § 12 for the helpers and the budget defaults.
- `error::Error` is `#[non_exhaustive]`: the compiler enforces a wildcard arm in any external matcher so adding a future variant (e.g., `RateLimited`) doesn't break callers. `Timeout` was added this way in PR-F4 without breaking existing matchers.
