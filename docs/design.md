...
- **Policy:** No regression from the base branch (`target: auto`, 0.5% threshold). New/changed code must be 80% covered (`patch: 80%`). All four statuses — backend project + patch, frontend project + patch — now report pass/fail rather than informational, so a regression surfaces a red ✗. **These are advisory until `main` carries a branch-protection rule requiring them:** the ✗ flags the problem but GitHub won't itself block the merge without that rule (none is configured today). (The frontend *project* status ran informational from PR-H2 — when the frontend was un-ignored but largely untested — until PR-H1's test-coverage backfill landed; the frontend *patch* status has reported since PR-H2.)
